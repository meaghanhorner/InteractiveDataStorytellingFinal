...
function drawStaticEpiChart(containerSelector, jsonPath) {
  ...
  .attr('d', makeAreaShading());
    ...
    svgTooltipG.append('rect')
      .attr('width', 180).attr('height', 85)
      .attr('fill', 'rgba(255, 255, 255, 0.96)')
      .attr('stroke', '#1a1a1a').attr('stroke-width', 1);

    const tTextYear = svgTooltipG.append('text').attr('x', 10).attr('y', 22).style('font-weight', 'bold').style('font-size', '12px').style('fill', '#333');
    const tTextProd = svgTooltipG.append('text').attr('x', 10).attr('y', 40).style('font-size', '11px').style('fill', '#008080');
    const tTextPay  = svgTooltipG.append('text').attr('x', 10).attr('y', 55).style('font-size', '11px').style('fill', '#d95f02');
    const tTextGap  = svgTooltipG.append('text').attr('x', 10).attr('y', 73).style('font-weight', 'bold').style('font-size', '11px').style('fill', '#1a1a1a');
  ...
  if (targetStep === 'step1' || targetStep === 'step2') {
    ...
    pOverlay
      ...
      .on('pointermove pointerenter', function(event) {
        ...
        const tooltipY      = mouseY - 40 < 0 ? mouseY + 15 : mouseY - 40;

        tTextYear.text(`${yr} ${qLabel}`);
        tTextProd.text(`Productivity: ${d.productivity.toFixed(1)}%`);
        tTextPay.text(`Hourly Pay: ${d.pay.toFixed(1)}%`);
        tTextGap.text(`Divergence: +${gap}%`);
...
function initChart(containerId, copy) {
  ...
  // draw is ONLY for chart D — no targetStep, no epi references
...
Promise.all([
  ...
  if (fraudContainer) {
    ...
    svg.append('text').attr('x', fm.left).attr('y', 46)
      .attr('font-size', '22px').attr('fill', '#666').attr('font-family', 'sans-serif')
